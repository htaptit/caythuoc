<?php
    ...
    $data = mysqli_get_record('tacdung', 'id', $id);
?>
<?php
    if($_SERVER['REQUEST_METHOD'] == 'POST') :
        ...
        if($_POST['tacdung'] && $_POST['tacdung'] != '' && !is_numeric($_POST['tacdung'])) :
            ...
            if(isset($str)) :
                if(is_valid_title_unique('tacdung', 'tentacdung', $str, 'id', $id)) :
                    $tacdung = $str;
                else :
        ...
        if(empty($errors)) :
            $query = "UPDATE tacdung SET tentacdung = '".$tacdung."' WHERE id = ".$id." LIMIT 1";
            $result = mysqli_query($dbc, $query);
            confirm_query($result, $query);
            if(mysqli_affected_rows($dbc) > 0) :
                $log_content = 'Chỉnh Sửa Thông Tin Tác Dụng : "'.$tacdung.'"';
                $args = array(
                    'id' => $_SESSION['userId'],
                    'content' => $log_content
                );
    ...
    <form method="post">
        <section id="formElement" class="widget form-box section">
            <div class="widget-title">
                ...
                <span>Chỉnh Sửa Thông Tin Tác Dụng</span>
            </div>
            <div class="row-fluid">
                <?php 
                    ...
                    <div class="span6 form-freeSpace">
                        <fieldset>
                            <div class="control-group">
                                ...
                                <div class="controls">
                                    <input class="small" type="text" name="tacdung" value="<?php if(isset($data['tentacdung'])) echo strip_tags($data['tentacdung']);?>" />
                                </div>
                            </div>
                        </fieldset>
                    </div>
